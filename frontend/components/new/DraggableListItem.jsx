...
export default function DraggableListItem({ item, index, deleteItem, itemListState }) {
...
function PollItem(props) {
    const {itemRef, itemsRef, itemListState, itemIndex, dragRef, dragHandleProps, draggableProps, isDragging} = props;
    const classes = pollItemStyles();
    ...
    return (
        <div ref={dragRef} 
            ...
            >
            <div className={classes.dragIconWrap} onClick={stopPropagation}><DragIndicatorIcon /></div>
            <div className={classes.descriptionWrap}>
                <TextField className={classes.inputDescription}
                    label=""
                    variant="outlined"
                    name="pollTitle"
                    fullWidth={true}
                    value={description}
                    error={isInvalid}
                    placeholder={"항목" + itemIndex}
                    helperText={isInvalid ? CommonValidator.pollItem.message : ""}
                    onChange={handleChange} 
                    />
                <div className={classes.deleteIconWrap} onClick={deleteItem}><ClearIcon  style={{ color: "rgba(255, 0, 0, 0.55)" }} /></div>
            </div>
